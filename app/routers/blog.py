...
@router.post("/", response_model=schemas.Blog, dependencies=[Depends(JWTBearer())])
def create_blog(
...
@router.get("/{blog_id}", response_model=schemas.Blog, dependencies=[Depends(JWTBearer())])
...
@router.delete("/{blog_id}", response_model=schemas.Blog, dependencies=[Depends(JWTBearer())])
def delete_blog(blog_id: int, db: Session = Depends(get_db)):
    db_blog = crud.delete_blog(db, blog_id=blog_id)
    return db_blog

@router.put("/{blog_id}", response_model=schemas.Blog, dependencies=[Depends(JWTBearer())])
def update_blog(
    blog_id: int,
