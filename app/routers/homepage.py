...
router = APIRouter(tags=["Home Services"], prefix="/home")
...
@router.post("/images", response_model=HomePageImageSchema, dependencies=[Depends(JWTBearer())])
def create_image(
...
@router.get("/images", response_model=List[HomePageImageSchema])
...
@router.get("/images/{image_id}", response_model=HomePageImageSchema, dependencies=[Depends(JWTBearer())])
...
@router.delete("/images/{image_id}", response_model=HomePageImageSchema, dependencies=[Depends(JWTBearer())])
def delete_image(image_id: int, db: Session = Depends(get_db)):
    db_image = homepage_crud.delete_image(db, image_id=image_id)
    return db_image

@router.put("/images/{image_id}", response_model=HomePageImageSchema, dependencies=[Depends(JWTBearer())])
def update_image(
    image_id: int,
